                           ...
                           int size, int rt, bool is_vector)
{
    ...
    if (s->pauth_active) {
        if (use_key_a) {
            gen_helper_autda(tcg_ctx, dirty_addr, tcg_ctx->cpu_env, dirty_addr, new_tmp_a64_zero(s));
        } else {
            gen_helper_autdb(tcg_ctx, dirty_addr, tcg_ctx->cpu_env, dirty_addr, new_tmp_a64_zero(s));
        }
    }
